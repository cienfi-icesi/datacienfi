...
download_notas <- function(url = "https://www.datos.gov.co/resource/kgxf-xxbe.csv?periodo=20224&$limit=50000") {
  ...
  datos_agregados <- datos %>%
    dplyr::group_by(cole_nombre_establecimiento) %>%
    dplyr::summarise(
      departamento = dplyr::first(cole_depto_ubicacion),
      municipio = dplyr::first(cole_mcpio_ubicacion),
      area = dplyr::first(cole_area_ubicacion),
      caracter = dplyr::first(cole_caracter),
      sector = dplyr::first(cole_naturaleza),
      bilingue = dplyr::first(cole_bilingue),
      ...
      total_estudiantes = dplyr::n(),
      .groups = "drop"  
    )
  
  # Paso 5: Filtrar instituciones con promedios en cero
  datos_agregados <- dplyr::filter(
    datos_agregados, 
    lectura_critica != 0,
    matematicas != 0,
  ...
  datos_agregados <- dplyr::rename(
    datos_agregados,
    nombre_colegio = cole_nombre_establecimiento
  )
